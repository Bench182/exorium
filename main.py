...
async def test621(ctx):
    embed = discord.Embed(color=config.color)
    embed.add_field(name="__**!Information!**__", value="Welcome to the **affiliates program**. The affiliates program of The Paw Kingdom allows for many type of individuals, or groups to engage into a partnership with The Paw Kingdom. Bringing perks and a nice community to those who wish to partner.\nWe partner with any of the following categories mentioned below:\n\n`-` charities & organisations\n`-` discord servers\n`-` companies\n`-` youtubers\n`-` twitch streamers\n`-` much more", inline=True)
    embed.add_field(name="__**Requirements**__", value="To make sure we have reasonable partners, we will judge whether or not we take partnerships by the following criteria. Please note that we have any right to deny partnerships, or to pull out of any at any time for any reason.\n\n__Companies/charities/organisations/etc.__\nWill be handled by a <@&724566501230641242>. Whether we accept the partnership is up to the person handling the partnership here as we can not state proper criteria per company, charity, organisation etc.\n\n__Discord servers__\n`-` 50+ members (including bots)\n`-` no NSFW exposure to minors\n`-` must not be anarchy\n`-` reasonably active (100+ msg/d)\n\n__youtube/twitch__\nWill depend on the twitch, or youtube channel content as well as the channel upload activeness. Subscrivers, average view and such do not impact your parthership application.\n\n[Click to go to the application](https://forms.gle/aR7NJFc3LpTMbrqF8)", inline=False)
    await ctx.send(embed=embed)
...
async def ban(ctx, member: discord.Member, *, reason=None):
    if 755072923886288927 > top_role:
        await ctx.send("The bot's role is too low (it's below the highest role the member has)")
